...
export const PROJECTS = [
  ...
  // ---------------------------------------------------------
  {
    ...
    year: "2024",
    desc:
      "Type a need — shelter, food, a clinic, legal aid — and Help Nearby returns verified options within a few miles, pulled from public datasets and refreshed weekly.",
    ...
    stack: [
      ...
      { group: "Data",    items: ["Open Referral schema", "211 feeds", "Custom normalizers"] },
      { group: "Frontend",items: ["Preact", "Vanilla CSS", "Leaflet"] },
      { group: "Ops",     items: ["GitHub Actions (cron)", "Docker", "Fly.io"] },
    ],
    screenshots: [
      { label: "Search results — list view", caption: "Mobile result list ordered by distance and freshness." },
      { label: "Map toggle", caption: "Same results overlaid on a coarse map for orientation." },
      { label: "Coverage report", caption: "Internal view: category density by ZIP — used to decide which feeds to add next." },
    ],
    learned: [
  ...
  // ---------------------------------------------------------
  {
    ...
    year: "2024 — present",
    desc:
      "An assistant for support agents. Drafts replies grounded in your docs, surfaces similar past tickets, and explains what it suggested and why. The agent ships; Sidecar learns.",
    ...
    stack: [
      { group: "AI",        items: ["Claude API (Sonnet for drafting, Haiku for classification)", "OpenAI embeddings", "Custom prompt layer with refusal contract"] },
      { group: "Backend",   items: ["TypeScript", "Node", "Postgres", "pgvector"] },
      { group: "Retrieval", items: ["Hybrid BM25 + vector", "Per-tenant index isolation", "Metadata filters"] },
      { group: "Frontend",  items: ["React", "Inbox-panel webview"] },
      { group: "Ops",       items: ["Structured JSON logs", "Loki + Grafana", "Offline eval harness", "Per-tenant rate limits"] },
    ],
    screenshots: [
      { label: "Draft pane in inbox", caption: "The agent's view: incoming ticket on the left, Sidecar's draft on the right, with the reasoning trace below." },
      { label: "Similar tickets panel", caption: "Three closest past tickets — scored, dated, click-through to the original thread." },
      { label: "Why this draft", caption: "Reasoning trace expanded — the exact doc passages and past tickets the draft was built from." },
      { label: "Discard digest", caption: "Weekly internal view: drafts agents threw away, grouped by category, used to find documentation gaps." },
      { label: "Eval harness output", caption: "Held-out test run — graded matches against golden replies, regressions flagged in red." },
    ],
    learned: [
  ...
  // ---------------------------------------------------------
  {
    ...
    year: "2025",
    desc:
      "Scans 14 markets every 30 seconds, tracks edges over a configurable threshold, logs every decision, runs a strict risk-budget kill-switch. Data and analytics — not financial advice.",
    ...
    stack: [
      ...
      { group: "Analytics", items: ["Metabase", "DuckDB rollups", "Custom report scripts"] },
      { group: "Ops",       items: ["systemd timers", "Loki logs", "Telegram bot for alerts"] },
    ],
    screenshots: [
      { label: "Scan loop overview", caption: "Real-time view of the 14 tracked markets with last-poll latency and error counts." },
      { label: "Decision log", caption: "Append-only log of every signal evaluation — including no-op decisions, with the exact inputs." },
      { label: "Risk-budget panel", caption: "Live caps and remaining budget for the day; the kill-switch state is always visible." },
      { label: "Backtest harness", caption: "Replay against the recorded time-series with the same code that runs live." },
      { label: "Weekly journal", caption: "Auto-generated review of policy soft-breaches and the conditions that produced them." },
    ],
    learned: [
  ...
  // ---------------------------------------------------------
  {
    ...
    year: "2025 — present",
    desc:
      "A local-first command system. Telegram-triggered, on-device LLM, runs scripted workflows across files, calendars, and notes. No cloud round-trip.",
    ...
    stack: [
      { group: "AI",         items: ["Ollama", "Llama 3.1 8B (local)", "Custom router prompt"] },
      { group: "Runtime",    items: ["Python", "FastAPI", "SQLite (audit log)"] },
      { group: "Integrations", items: ["Telegram Bot API", "CalDAV", "Local notes (Markdown vault)"] },
      { group: "Hardware",   items: ["Home server", "Raspberry Pi (relay)"] },
    ],
    screenshots: [
      { label: "Telegram thread", caption: "Day-in-the-life: short messages in, confirmed actions out." },
      { label: "Tool router output", caption: "Internal view: how a single message is routed into a typed tool call." },
      { label: "Audit log", caption: "Append-only record of every command and the state change it produced." },
    ],
    learned: [
